...
class FinnScraper:
    ...
    def scrape_data(self, old=False):
        ...
        while True:
            ...
            if "Beklager, nå er det rusk i maskineriet" in error_found:
                break
            if "Ingen treff akkurat nå" in no_hits_error:
                break
    ...
    def scrape_single_listing(self, url):
        ...
        try:
            ...
            price = int(str(soup.select_one("span.u-t3").text).replace("kr","").replace(" ", "").replace(u'\xa0', ""))
            ...
            data = {
                ...
                "title":str(title), 
                "description":str(description).replace("<p><br/></p>", "").replace("<br/><br/>", "<br/>").replace("<p><b>&nbsp;</b></p>", "").replace("<b>", "").replace("</b>", ""), 
            ...
            try:
                ...
                if int(price) != int(listing.orignal_price):
                    if not Price_History.objects.filter(listing=finn_kode,price_changed_date=last_changed, changed_price=price).exists():
                        print("Price Changed Now Updating...")
                        Price_History.objects.create(changed_price=price, price_changed_date=last_changed, listing=listing)
                        
            except Listing.DoesNotExist:

                Listing.objects.create(finn_code=int(finn_kode), title=str(title), description=str(description), Boat_location=str(Boat_location), State=str(State), Type=str(Type), Brand=str(Brand), Model=str(Model), Model_Year=str(Model_Year), Length_feet=str(Length_feet), Length_cm=str(Length_cm), Width=str(Width), Depth=str(Depth), Engine_Included=str(Engine_Included), Engine_Manufacturer=str(
                                        Engine_Manufacturer), Engine_Type=str(Engine_Type), Motorstr=str(Motorstr), Max_Speed=str(Max_Speed), Fuel=str(Fuel), Weight=str(Weight), Material=str(Material), Color=str(Color), Seating=str(Seating), Sleeps=str(Sleeps), orignal_price=int(price), last_changed=str(last_changed), contact_name=str(contact_name), phone_number=str(phone_number), address=str(address), image=str(image), last_updated=str(timestamp), url=str(url), status=str(status))
